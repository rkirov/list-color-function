...
`ListColoring` library of this repository, under exactly those names, so
importing the library is the whole submission — no re-export shim is needed, and none is wanted: a
shim would put a second declaration between the comparator and the thing that was actually proved.
