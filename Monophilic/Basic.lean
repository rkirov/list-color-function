...
/-!
# Basic theory of `col` and monophilicity
...
* `SimpleGraph.monophilic_of_not_colorable` : if `n < χ(G)` then `G` is `n`-monophilic
* `SimpleGraph.not_monophilic_of_colorable_of_not_choosable` : if `χ(G) ≤ n < χ_ℓ(G)` then `G` is
  not `n`-monophilic
...
/-- Item (2): if `n < χ(G)` then `G` is `n`-monophilic, vacuously — there are no colorings at all
from the constant list. -/
theorem monophilic_of_not_colorable {n : ℕ} (h : ¬ G.Colorable n) : G.Monophilic n := by
  intro L _
  rw [colConst_eq_zero_iff_not_colorable.mpr h]
  exact Nat.zero_le _

/-- Item (3): if `χ(G) ≤ n < χ_ℓ(G)` then `G` is *not* `n`-monophilic. -/
theorem not_monophilic_of_colorable_of_not_choosable {n : ℕ}
    (hcol : G.Colorable n) (hch : ¬ G.Choosable n) : ¬ G.Monophilic n := by
  intro hmono
  rw [Choosable] at hch
  push Not at hch
...
/-- Conversely, an `n`-monophilic `n`-colorable graph is `n`-choosable. -/
theorem choosable_of_monophilic_of_colorable {n : ℕ}
    (hmono : G.Monophilic n) (hcol : G.Colorable n) : G.Choosable n :=
