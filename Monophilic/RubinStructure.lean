...
* `Monophilic.exists_connecting_path` — E2: Rubin's connecting path, for an arbitrary set of
  vertices and set of already-used edges
* `Monophilic.exists_connecting_path_of_cycle` — E2 as Rubin states it, for a cycle `C₁`
* `Monophilic.not_choosable_two_of_cycles_meet_le_one` — Rubin's step 3, from Mathlib's cycles
* `Monophilic.exists_connecting_path_of_choosable`,
  `Monophilic.exists_connecting_path_of_cycle_of_choosable` — E2 with its cycle hypothesis
...
finite pigeonhole behind `Monophilic.exists_incident_edge_notMem`; not mathematics. -/
...
The specialization of `Monophilic.exists_connecting_path` to `S` the vertices and `F` the edges of
...
The two cases are one theorem here (`Monophilic.not_choosable_two_of_dumbbell`, whose length-`0`
...
The point of proving this here is that it converts the hypothesis of `exists_connecting_path` —
"no cycle meets `S` in at most one vertex" — into the hypothesis Rubin's proof actually has at
that point, namely that `G` is `2`-choosable. That composite is
`Monophilic.exists_connecting_path_of_choosable`, and it is the form his steps 4 and 5 both use. -/
...
node), which `Monophilic.not_choosable_two_of_dumbbell` covers together.
...
node. Combining `Monophilic.exists_connecting_path` with
`Monophilic.not_choosable_two_of_cycles_meet_le_one` is what turns one into the other; note that a
...
The specialization of `Monophilic.exists_connecting_path_of_choosable` to `S` and `F` the vertices
...
`Monophilic.RubinTheorem` is **not** discharged; steps 4–6 of Rubin's proof and the assembly are
...
* **Cases (i) and (ii) of Rubin's six cannot occur.** Both describe a `P₂` producing a cycle that
  ...
  hypothesis `Monophilic.exists_connecting_path_of_choosable` already carries. Neither case fires
...
* **Case (v) needs `P₁` to be shortest.** With `P₂` joining the two branch vertices, the four arms
  ...
  whatever their lengths, which `Monophilic.not_choosable_two_gtheta_of_four` covers.
...
graph structure to `Monophilic.CoreIs`: that every connected graph *has* a core (a witness for
`Monophilic.CoreIs G H` with `H` of minimum degree `≥ 2`), and the two isomorphism constructions
`H ≃g closePath k` from `H` being connected and `2`-regular and `H ≃g theta m` from `H` being
`C₁ ∪ P₁` and nothing more. Steps 4–6 also need `Monophilic.Contains G (gtheta ks)` built from
walk data, which the abstract theta results
(`Monophilic.choosable_two_gtheta_iff`, `Monophilic.not_choosable_two_gtheta_of_four`) consume. -/

#print axioms Monophilic.exists_isCycle_of_two_le_degree
#print axioms Monophilic.exists_shortest_isCycle
#print axioms Monophilic.not_choosable_two_of_two_cycle_walks
#print axioms Monophilic.exists_boundary_adj
#print axioms Monophilic.exists_incident_edge_notMem
#print axioms Monophilic.exists_connecting_path
#print axioms Monophilic.exists_connecting_path_of_cycle
#print axioms Monophilic.not_choosable_two_of_cycles_meet_le_one
#print axioms Monophilic.exists_connecting_path_of_choosable
#print axioms Monophilic.exists_connecting_path_of_cycle_of_choosable
