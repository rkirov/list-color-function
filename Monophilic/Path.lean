...
and the deletion identity of `Monophilic.Delete` applies with no transport.
...
* `Monophilic.PathV k`, `Monophilic.pathG k` : the vertex type and graph of a path of length `k`
* `Monophilic.pathEnd k`, `Monophilic.pathStart k` : its two terminal vertices

## Main results

* `Monophilic.col_pathG_succ` : peeling the endpoint off a path,
