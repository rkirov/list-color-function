...
**every chordal graph is `n`-monophilic for every `n`**. The proof is an induction along a
*simplicial elimination ordering*: order the vertices `v₁, …, v_m` so that each `v_i` is simplicial
in the graph induced by `v₁, …, v_i`, i.e. its neighbours among the earlier vertices form a clique.
Reading the ordering backwards, the graph is built from the empty graph by repeatedly adding a new
vertex joined to a clique of the graph constructed so far — and each such step preserves
`n`-monophilicity by Lemma 1 (`SimpleGraph.Monophilic.coneOn`).
...
As in `Monophilic.Core`, adding a vertex enlarges the vertex type by one point, so after `k` steps
the vertex type is `SimpleGraph.TowerV V k` (`V` with `k` extra points, reused verbatim from
`Monophilic.Core`) and the construction data is a nested tuple recording, at each stage, the set of
old vertices the new vertex is joined to (`SimpleGraph.CliqueTowerData`). This generalizes
`SimpleGraph.pendantTower`, where every attachment set is a singleton, to arbitrary attachment
sets; the hypothesis that each of them is a clique *in the graph existing at that stage* is
...
* `SimpleGraph.monophilic_cliqueTower` : a simplicial tower over an `n`-monophilic graph is
  `n`-monophilic
* `SimpleGraph.monophilic_cliqueTower_of_isEmpty` : **the Kostochka–Sidorenko corollary**, in its
  simplicial-elimination-ordering form: a graph built by a simplicial tower over the empty graph is
  `n`-monophilic for every `n`
* `SimpleGraph.monophilic_pendantTower_of_cliqueTower` : a pendant tower is the special case in
  which every attachment set is a singleton
* `SimpleGraph.monophilic_top_fin_of_cliqueTower` : complete graphs recovered from the corollary
-/
...
of cone attachments over `G`, then the graph `cliqueTower G k d` built by the tower is
`n`-monophilic.

The induction is on the height `k` of the tower; each step is Kirov–Naimi's Lemma 1
(`SimpleGraph.Monophilic.coneOn`) applied to the clique `d.2`. -/
theorem monophilic_cliqueTower {n : ℕ} (hG : G.Monophilic n) :
  ...
  | k + 1, d, h =>
      ...
      letI := instDecidableRelCliqueTower (G := G) k d.1
      Monophilic.coneOn h.1 (monophilic_cliqueTower hG k d.1 h.2)

/-- **The Kostochka–Sidorenko corollary** (Kirov–Naimi, Corollary to Lemma 1), in its simplicial
elimination ordering form: *a graph admitting a simplicial elimination ordering is `n`-monophilic
for every `n`*.
...
`SimpleGraph.monophilic_of_isEmpty`, the step is Lemma 1.
...
"every chordal graph is `n`-monophilic". Dirac's theorem is a statement about graph structure
alone, involves no colorings, and is **not part of this development**. -/
theorem monophilic_cliqueTower_of_isEmpty {V : Type u} [Fintype V] [DecidableEq V] [IsEmpty V]
    ...
    (hd : CliqueTowerData.IsSimplicial G k d) : (cliqueTower G k d).Monophilic n :=
  monophilic_cliqueTower (monophilic_of_isEmpty G n) k d hd
...
clique is `n`-monophilic, for every `n`. -/
theorem monophilic_cliqueTower_fin_zero (n k : ℕ) (d : CliqueTowerData (Fin 0) k)
    (hd : CliqueTowerData.IsSimplicial (⊥ : SimpleGraph (Fin 0)) k d) :
    (cliqueTower (⊥ : SimpleGraph (Fin 0)) k d).Monophilic n :=
  monophilic_cliqueTower_of_isEmpty _ n k d hd
...
of pendant attachments over an `n`-monophilic graph is `n`-monophilic, proved here through
`SimpleGraph.monophilic_cliqueTower` rather than through
`SimpleGraph.monophilic_pendantTower_iff`. -/
theorem monophilic_pendantTower_of_cliqueTower {n : ℕ} (hG : G.Monophilic n) (k : ℕ)
    (d : TowerData V k) : (pendantTower G k d).Monophilic n := by
  letI := instDecidableEqTowerV (V := V) k
  letI := instFintypeTowerV (V := V) k
  letI := instDecidableRelPendantTower (G := G) k d
  letI := instDecidableRelCliqueTower (G := G) k (TowerData.toCliqueTowerData k d)
  refine (monophilic_congr (G := pendantTower G k d)
    (G' := cliqueTower G k (TowerData.toCliqueTowerData k d)) (fun a b => ?_) n).mpr
    (monophilic_cliqueTower hG k _ (isSimplicial_toCliqueTowerData G k d))
...
so `SimpleGraph.monophilic_top_fin` is itself a special case of the corollary. -/
...
clique) set of all earlier vertices, so `SimpleGraph.monophilic_top_fin` follows from
`SimpleGraph.monophilic_cliqueTower_of_isEmpty`. -/
theorem monophilic_top_fin_of_cliqueTower (m n : ℕ) : (⊤ : SimpleGraph (Fin m)).Monophilic n := by
  letI := instDecidableEqTowerV (V := Fin 0) m
  letI := instFintypeTowerV (V := Fin 0) m
  have h : (cliqueTower (⊥ : SimpleGraph (Fin 0)) m (univTowerData (Fin 0) m)).Monophilic n :=
    monophilic_cliqueTower_of_isEmpty _ n m _ (isSimplicial_univTowerData m)
  have htop : (⊤ : SimpleGraph (TowerV (Fin 0) m)).Monophilic n :=
    (monophilic_congr (G := cliqueTower (⊥ : SimpleGraph (Fin 0)) m (univTowerData (Fin 0) m))
      (G' := (⊤ : SimpleGraph (TowerV (Fin 0) m)))
      (fun a b => by rw [cliqueTower_univTowerData m]) n).mp h
  exact (monophilic_iso (topIsoTopOfEquiv (towerVEquivFin m)) n).mp htop
