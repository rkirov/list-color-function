...
(`Monophilic.min_pathA_pathB_le_col`) bounds its count from below by `min (A, B)`. That is the
content of `Monophilic.min_pathA_pathB_le_col_inducedList`.

Since `col(C, n) = n · A` (`Monophilic.colConst_closePath_succ`), this already settles the case
...
(`Monophilic.monophilic_closePath_of_odd`).
...
(`Monophilic.isPathShape_parity_of_minimizing`) and Lemma 4 (`Monophilic.col_lt_col_of_ssubset`)
together force `M_{c₀}` to be *exactly* a type B `(n, n-1)`-assignment
(`Monophilic.isPathShape_of_col_lt`), with palette `S`, and `c₀ ∉ S` while `c₀` belongs to both of
...
Following `Monophilic.Cycle`, `closePath (j + 1)` is the cycle on `j + 2` vertices, and deleting one
vertex leaves the path `pathG j` of length `j`. So the paper's cycle of length `k` is
...
* `Monophilic.min_pathA_pathB_le_col_inducedList` : the reduction of a cycle to a path
* `Monophilic.monophilic_closePath_of_odd` : **odd cycles are `n`-monophilic for every `n ≥ 2`**
* `Monophilic.monophilic_closePath_of_even` : **even cycles are `n`-monophilic for every `n ≥ 3`**
* `Monophilic.monophilic_closePath` : **every cycle is `n`-monophilic for every `n ≥ 3`**
...
color `3` leaves on the remaining path a genuine **type B** `(3,2)`-assignment — palette `{0,1,2}`,
missing `0` at one end and `1` at the other — so that fibre contributes only `B₂ = 5`, one less than
`A₂ = 6`. This is exactly the situation `Monophilic.isPathShape_of_col_lt` isolates; note the color
`3` lies outside the palette and inside both terminal lists, as the proof of Case 2 predicts. -/
...
/-- **Case 1 of Theorem 1: cycles with an odd number of vertices are `n`-monophilic, for every
`n ≥ 2`.**
...
theorem monophilic_closePath_of_odd (hj : Odd j) (m : ℕ) :
    (closePath (j + 1)).Monophilic (m + 2) := by
  intro M hM
  have hj1 : 1 ≤ j := hj.pos
...
Both halves of the paper's argument are used: Lemma 4 (`Monophilic.col_lt_col_of_ssubset`) rules out
that `N` is strictly larger than the `(n, n-1)`-assignment inside it, and Lemma 3(c)
(`Monophilic.isPathShape_parity_of_minimizing`) supplies the type. -/
...
/-- **Case 2 of Theorem 1: cycles with an even number of vertices are `n`-monophilic, for every
`n ≥ 3`.**
...
theorem monophilic_closePath_of_even (hj : 2 ≤ j) (hev : Even j) (hm : 1 ≤ m) :
    (closePath (j + 1)).Monophilic (m + 2) := by
  intro M hM
...
/-- **Theorem 1 of Kirov–Naimi, for `n ≥ 3`: every cycle is `n`-monophilic.**
...
here; see `Monophilic.monophilic_closePath_of_odd`. -/
theorem monophilic_closePath_succ (hj : 1 ≤ j) (hm : 1 ≤ m) :
    (closePath (j + 1)).Monophilic (m + 2) := by
  rcases Nat.even_or_odd j with hev | hodd
  · exact monophilic_closePath_of_even (by rcases hev with ⟨r, hr⟩; omega) hev hm
  · exact monophilic_closePath_of_odd hodd m

/-- **Theorem 1 of Kirov–Naimi, for `n ≥ 3`**, indexed as in `Monophilic.colConst_closePath`:
`closePath k` is the cycle on `k + 1` vertices, and `2 ≤ k` says it has at least three. -/
theorem monophilic_closePath {k : ℕ} (hk : 2 ≤ k) (hm : 1 ≤ m) :
    (closePath k).Monophilic (m + 2) := by
  obtain ⟨i, rfl⟩ : ∃ i, k = i + 1 := ⟨k - 1, by omega⟩
  exact monophilic_closePath_succ (by omega) hm
