...
Writing `n = m + 2` throughout (as in `Monophilic.Recurrence`), `pathAssign k n x y` is the
...
* `Monophilic.inducedList_pathAssign` : the recursion on list assignments
* `Monophilic.col_pathAssign` : `col(P_k, L) = A_k` for type A and `B_k` for type B
-/
