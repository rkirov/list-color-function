...
`Monophilic` are isomorphism invariants.

This is pure transport, but it is what lets an argument that builds a graph up structurally (as in
`Monophilic.Cone`, where a new vertex is added as an `Option`) be applied to a concretely indexed
...
* `SimpleGraph.monophilic_iso` : `G` is `n`-monophilic iff `G'` is
-/
...
/-! ### Invariance of monophilicity -/

/-- **Monophilicity is an isomorphism invariant.** -/
theorem monophilic_iso (e : G ≃g G') (n : ℕ) : G.Monophilic n ↔ G'.Monophilic n := by
  constructor
  · intro h M hM
