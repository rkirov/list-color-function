...
Let `G` be an `n`-monophilic graph and let `v₁, …, v_k` induce a complete subgraph of `G`. Let `G'`
be obtained from `G` by adding one new vertex joined to `v₁, …, v_k`. Then `G'` is `n`-monophilic.
This is Lemma 1 of Kirov–Naimi, and it is the engine behind the Kostochka–Sidorenko theorem
(a graph with a simplicial elimination ordering is `n`-monophilic for every `n`).
...
* `SimpleGraph.Monophilic.coneOn` : **Lemma 1**
* `SimpleGraph.monophilic_top` : complete graphs are `n`-monophilic for every `n`
-/
...
theorem Monophilic.coneOn (hK : G.IsClique (K : Set V)) {n : ℕ} (hG : G.Monophilic n) :
    (SimpleGraph.coneOn G K).Monophilic n := by
  intro M hM
...
/-! ### Corollary: complete graphs are `n`-monophilic -/

section Complete

/-- A graph on an empty vertex type is `n`-monophilic for every `n`: from any list assignment there
is exactly one coloring, the empty function. This is the base case of the induction below. -/
theorem monophilic_of_isEmpty {V : Type*} [Fintype V] [DecidableEq V] [IsEmpty V]
    (G : SimpleGraph V) [DecidableRel G.Adj] (n : ℕ) : G.Monophilic n := by
  intro L _
...
/-- **Complete graphs on `Fin m` are `n`-monophilic, for all `m` and `n`.** The induction step
is Lemma 1 applied to the clique consisting of all the old vertices. -/
theorem monophilic_top_fin (m n : ℕ) : (⊤ : SimpleGraph (Fin m)).Monophilic n := by
  induction m with
  | zero => exact monophilic_of_isEmpty _ n
  | succ m ih =>
    have hK : (⊤ : SimpleGraph (Fin m)).IsClique ((univ : Finset (Fin m)) : Set (Fin m)) := by
      intro a _ b _ hab
      exact hab
    exact (monophilic_iso (topIsoConeOnTop m) n).mpr (Monophilic.coneOn hK ih)
...
theorem monophilic_top (V : Type*) [Fintype V] [DecidableEq V] (n : ℕ) :
  ...
  (monophilic_iso (topIsoTopFin V) n).mpr (monophilic_top_fin _ n)
