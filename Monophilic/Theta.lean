...
/-!
# Theta graphs are not `2`-monophilic

Kirov–Naimi's Theorem 2 characterizes the `2`-monophilic graphs. Its converse runs through
**Rubin's theorem** (Erdős–Rubin–Taylor 1980): a connected graph is `2`-choosable iff its core is a
single vertex, an even cycle, or `θ_{2,2,2m}`. Rubin's theorem is *not* proved here (it is not
available in any proof assistant); what this file supplies is the one remaining graph-theoretic
input, namely that the last family in Rubin's list is `2`-monophilic only in its first member:

> for `m ≥ 2`, `θ_{2,2,2m}` is **not** `2`-monophilic.

(For `m = 1` the graph `θ_{2,2,2}` is `K₂,₃`, which *is* `2`-monophilic; that is Lemma 6.)
...
vertices — see `Monophilic.theta`. That pair is not a clique, so `colConst_coneOn` does not apply,
but `col_coneOn` needs no clique hypothesis and iterating it gives the product formula
`Monophilic.col_theta`.

## The witness

`Monophilic.thetaWitness` gives the lists `{1,2}` and `{2,3}` to the two added vertices, `{1,2}` to
`pathEnd`, `{1,3}` to `pathStart`, and along the long path, read from the `pathEnd` side,
`{1,2}` repeated `2m-3` times, then `{2,3}`, then `{1,3}` (this is `Monophilic.lvl`, indexed by the
...
alternating chain `1, 2, 1, 2, …` arrives at the far end with its only remaining color already
used. The fourth term contributes `1`, whereas `col(θ_{2,2,2m}, 2) = 2`.

## Main definitions

* `Monophilic.theta m` : the theta graph `θ_{2,2,2m}`
* `Monophilic.levelAssign` : a list assignment on a path given by a level function, with the two
  terminal lists overridden
* `Monophilic.thetaWitness m` : the witness `2`-list assignment
...
* `Monophilic.col_theta` : **the product formula** for `col(θ_{2,2,2m}, M)`
* `Monophilic.colConst_theta` : `col(θ_{2,2,2m}, 2) = 2` for `m ≥ 1`
* `Monophilic.col_theta_witness` : `col(θ_{2,2,2m}, L) = 1` for the witness `L`, when `m ≥ 2`
* `Monophilic.not_monophilic_theta` : **`θ_{2,2,2m}` is not `2`-monophilic for `m ≥ 2`**
* `SimpleGraph.monophilic_two_iff_of_rubin` : **Theorem 2**, stated relative to Rubin's theorem,
...
-- constant list: `K₂,₃` is `2`-monophilic (Lemma 6). The hypothesis `m ≥ 2` is not an artifact.
...
theorem not_monophilic_theta (m : ℕ) (hm : 2 ≤ m) : ¬ (theta m).Monophilic 2 := by
  intro hmono
...
the only facts used about them are the four monophilicity inputs, each of which *is* proved in this
development, modulo Lemma 5 (the core reduction, `Monophilic.Core`):

* `hvertex` — a graph whose core is a single vertex is a tree, hence `n`-monophilic for every `n`
  (Kostochka–Sidorenko, `Monophilic.Cone`);
* `hcycle` — cycles are `n`-monophilic for every `n ≥ 2` (Theorem 1,
  `Monophilic.CycleMonophilic`);
* `hK23` — `θ_{2,2,2} = K₂,₃` is `2`-monophilic (Lemma 6, `Monophilic.K23`);
* `htheta` — `θ_{2,2,2m}` is *not* `2`-monophilic for `m ≥ 2` (`Monophilic.not_monophilic_theta`,
  this file).

The conclusion is the Kirov–Naimi characterization: a connected graph is `2`-monophilic exactly
when it is not `2`-colorable (it then has *no* coloring from the constant list, so the inequality
holds vacuously — this is the odd-cycle case) or its core is a single vertex, an even cycle, or
`K₂,₃`. -/
theorem monophilic_two_iff_of_rubin {V : Type*} [Fintype V] [DecidableEq V]
  ...
  · intro hmono
    ...
    · rcases rubin.mp (choosable_of_monophilic_of_colorable hmono hcol) with h | h | ⟨m, hm, h⟩
