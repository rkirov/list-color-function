...
assignments differ, and that is exactly `Monophilic.colFix_pos_of_two_le_card`.
...
* `Monophilic.col_pos_of_one_le_card_pathEnd` : a path can be colored from any list assignment with
  at least one color at `pathEnd` and at least two colors everywhere else
* `Monophilic.colFix_pos_of_two_le_card` : with at least two colors at every vertex, a path can be
  colored with any prescribed color at any prescribed vertex
* `Monophilic.col_lt_col_of_ssubset` : **Lemma 4** of Kirov–Naimi
-/
