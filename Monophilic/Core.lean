...
The **core** of a graph is what is left after repeatedly deleting vertices of degree `1` until
every remaining vertex has degree at least `2`. Kirov–Naimi, Lemma 5, states that a connected graph
is `n`-monophilic if and only if its core is. Since the core is reached by finitely many pendant
deletions, the whole content of the lemma is a *single pendant step*, proved here in both
directions.

The pendant step is an instance of coning off a clique: a pendant vertex is a new vertex joined to
the singleton `{v}`, and a singleton is always a clique. So the forward direction is exactly
Lemma 1 (`SimpleGraph.Monophilic.coneOn`). The converse is the new content: given an `n`-list
assignment `L` on `G`, extend it to the cone by giving the *new* vertex the same list as its
neighbour `v`. Every coloring `f` of `G` from `L` then uses a color `f v` that the apex must avoid
and which is present in the apex list, so the apex has exactly `n - 1` free colors above every
`f`, giving `col(coneOn G {v}, M) = (n - 1) * col(G, L)` on the nose. Comparing this with
`col(coneOn G {v}, n) = (n - 1) * col(G, n)` and cancelling `n - 1` yields `G`'s monophilicity.

Cancelling `n - 1` needs `n ≠ 1`, and truncated subtraction needs `n ≠ 0`; but this is not a real
restriction, because *every* graph is `n`-monophilic for `n ≤ 1`
(`SimpleGraph.monophilic_of_le_one`). The equivalence is therefore proved for all `n`.
...
is how Lemma 5 is actually used: `SimpleGraph.pendantTower G k d` is the graph obtained from `G`
by attaching `k` pendant vertices in succession, each one at an arbitrary vertex of the graph built
so far, and `SimpleGraph.monophilic_pendantTower_iff` says that this graph is `n`-monophilic iff
`G` is. Taking `G` to be the core of a graph `H` and the tower to be the reversed sequence of
pendant deletions leading from `H` to `G` gives Lemma 5.
...
* `SimpleGraph.monophilic_of_le_one` : every graph is `n`-monophilic for `n ≤ 1`
* `SimpleGraph.Monophilic.addPendant` : the forward pendant step
* `SimpleGraph.monophilic_of_monophilic_coneOn_singleton` : the converse pendant step
* `SimpleGraph.monophilic_coneOn_singleton_iff`, `SimpleGraph.monophilic_addPendant_iff` : the two
  directions packaged as an `iff`, for every `n`
* `SimpleGraph.monophilic_pendantTower_iff` : the iterated form, **Lemma 5**
-/
...
/-- Monophilicity depends only on the adjacency relation. -/
theorem monophilic_congr {G' : SimpleGraph V} [DecidableRel G'.Adj]
    (h : ∀ a b, G.Adj a b ↔ G'.Adj a b) (n : ℕ) : G.Monophilic n ↔ G'.Monophilic n := by
  unfold Monophilic
...
omit [Fintype V] [DecidableEq V] [DecidableRel G.Adj] in
/-- A singleton is a clique, in the `Finset`-coerced form required by `Monophilic.Cone`. -/
...
/-! ### Every graph is `n`-monophilic for `n ≤ 1`
...
/-- **Every graph is `n`-monophilic for `n ≤ 1`.** For `n = 0` there is a coloring from the
constant list only when `V` is empty, and for `n = 1` only when `G` has no edges; in both cases
every `n`-list assignment admits a coloring as well, and the constant count is at most one. -/
theorem monophilic_of_le_one {n : ℕ} (hn : n ≤ 1) : G.Monophilic n := by
  intro L hL
...
/-- **The forward pendant step.** If `G` is `n`-monophilic then so is `G` with a new pendant vertex
attached at `v`. This is Lemma 1 of Kirov–Naimi applied to the singleton clique `{v}`. -/
theorem Monophilic.addPendant {n : ℕ} (hG : G.Monophilic n) (v : V) :
    (SimpleGraph.coneOn G {v}).Monophilic n :=
  Monophilic.coneOn (isClique_coe_singleton v) hG

/-- The forward pendant step, phrased for `SimpleGraph.addPendant`. -/
theorem Monophilic.addPendant' {n : ℕ} (hG : G.Monophilic n) (v : V) :
    (G.addPendant v).Monophilic n :=
  (SimpleGraph.monophilic_congr (fun _ _ => by rw [SimpleGraph.addPendant_eq_coneOn v]) n).mpr
    (Monophilic.addPendant hG v)

/-- **The converse pendant step** (the new content of Lemma 5). If attaching a pendant vertex at
`v` gives an `n`-monophilic graph, then `G` itself is `n`-monophilic.

Given an `n`-list assignment `L` on `G`, extend it to the cone by repeating the list of `v` at the
apex. Monophilicity of the cone reads `(n - 1) * col(G, n) ≤ (n - 1) * col(G, L)` by
`colConst_coneOn_singleton` and `col_coneOn_singleton_extend`, and `n - 1` cancels once `2 ≤ n`.
For `n ≤ 1` the conclusion is automatic (`monophilic_of_le_one`), which is why
`monophilic_coneOn_singleton_iff` below needs no hypothesis on `n`. -/
theorem monophilic_of_monophilic_coneOn_singleton {n : ℕ} (hn : 2 ≤ n) (v : V)
    (h : (coneOn G {v}).Monophilic n) : G.Monophilic n := by
  intro L hL
...
theorem monophilic_of_monophilic_addPendant {n : ℕ} (hn : 2 ≤ n) (v : V)
    (h : (G.addPendant v).Monophilic n) : G.Monophilic n :=
  monophilic_of_monophilic_coneOn_singleton hn v
    ((monophilic_congr (fun a b => by rw [addPendant_eq_coneOn v]) n).mp h)

/-- **The pendant step, Kirov–Naimi Lemma 5 for a single vertex of degree one.** Attaching a
pendant vertex preserves and reflects `n`-monophilicity.

No hypothesis on `n` is needed: for `n ≤ 1` both sides hold outright by
`monophilic_of_le_one`. -/
theorem monophilic_coneOn_singleton_iff (v : V) (n : ℕ) :
    (coneOn G {v}).Monophilic n ↔ G.Monophilic n := by
  refine ⟨fun h => ?_, fun h => Monophilic.addPendant h v⟩
  by_cases hn : 2 ≤ n
  · exact monophilic_of_monophilic_coneOn_singleton hn v h
  · exact monophilic_of_le_one (by omega)

/-- The pendant step for `SimpleGraph.addPendant`. -/
theorem monophilic_addPendant_iff (v : V) (n : ℕ) :
    (G.addPendant v).Monophilic n ↔ G.Monophilic n := by
  rw [monophilic_congr (G' := coneOn G {v}) (fun _ _ => by rw [addPendant_eq_coneOn v]) n]
  exact monophilic_coneOn_singleton_iff v n
...
far, possibly to an earlier new vertex — changes neither the `n`-monophilicity of `G` nor its
failure. Applying this with `G` the core of a graph `H` and the tower the sequence of pendant
deletions leading from `H` down to `G` (reversed) gives Lemma 5 as stated in the paper.
...
is `n`-monophilic if and only if `G` is.
...
paper: `H` is `n`-monophilic iff its core is. -/
theorem monophilic_pendantTower_iff (n : ℕ) :
    ∀ (k : ℕ) (d : TowerData V k), (pendantTower G k d).Monophilic n ↔ G.Monophilic n
  | 0, _ => Iff.rfl
  | k + 1, d =>
      letI := instDecidableEqTowerV (V := V) k
      letI := instFintypeTowerV (V := V) k
      letI := instDecidableRelPendantTower (G := G) k d.1
      (monophilic_addPendant_iff (G := pendantTower G k d.1) d.2 n).trans
        (monophilic_pendantTower_iff n k d.1)
...
graph, so `monophilic_pendantTower_iff` applies to it. -/
