...
`Monophilic.RubinStructure` supplies Rubin's steps 1–3 and the existence of his connecting paths.
This file supplies the two remaining pieces.

* **The theta bridge.** The choosability results of `Monophilic.ThetaGen` are stated about the
  abstract graph `Monophilic.gtheta ks`, while Rubin's steps produce *walks*.
  `Monophilic.contains_gtheta_of_walks` converts `n` internally disjoint paths joining two
  branch vertices into `Monophilic.Contains G (gtheta ks)`, which
  `Monophilic.not_choosable_of_contains` then consumes. This is the analogue for thetas of the
  `getVert` bridge at the top of `Monophilic.RubinStructure`, and like it is pure mechanization.
...
`Monophilic.RubinStructure`; this file respects them.
...
1. **Cases (i) and (ii) cannot occur** — each produces a cycle meeting `C₁` in at most one node,
   ...
   `Monophilic.case_onP1`, which is cheaper than re-deriving the step-3 hypothesis.
2. **Case (v)'s sub-split is unnecessary, and without minimality of `P₁` it is wrong** — the arms
   `2, 2, 4, 2` contain no bad three-arm theta. `Monophilic.case_branches` uses Rubin's type `5`
   unconditionally instead: four arms of any lengths.
3. **Step 5's "`C₁` must be a `4`-cycle" needs minimality of `C₁`**, which enters
   `Monophilic.step4` through the hypothesis `hmin`.

## Main results

* `Monophilic.contains_gtheta_of_arms`, `Monophilic.contains_gtheta_of_walks` — the bridge
* `Monophilic.not_choosable_two_of_three_arms`, `Monophilic.not_choosable_two_of_four_arms` —
  Rubin's types `4` and `5`, stated over arm data
* `Monophilic.step4` — Rubin's steps 4 and 5: `C₁ ∪ P₁` is `θ_{2,2,2m}` and `C₁` is a four-cycle
* `Monophilic.case_onP1`, `Monophilic.case_interior`, `Monophilic.case_mixed`,
  `Monophilic.case_branches`, `Monophilic.case_middles` — Rubin's six cases (his (i) and (ii)
  merged into the first)
* `Monophilic.step56` — the case analysis assembled: no edge lies outside `C₁ ∪ P₁`
* `Monophilic.rubin_structure` — **the structural half of Rubin's theorem**: a connected,
...
/-! ### Decoding the index layout of `Monophilic.gtheta` -/

/-- Which arm the interior index `x` of `Θ(k₁, …, k_n)` belongs to. Pure bookkeeping: the inverse
of the running-offset layout `Monophilic.gsize (ks.take i) + j`. -/
...
/-- `Monophilic.armStepB`, read as a proposition. -/
...
`Monophilic.gAdjAux_of_armStepB`. -/
...
Rubin's steps 4–6 produce **walks**, while the choosability results of `Monophilic.ThetaGen` are
stated about the abstract graph `Monophilic.gtheta ks`. This section is the bridge, and it is
**mechanization only**: it re-indexes `n` internally disjoint paths joining two vertices onto the
layout `Fin (gsize ks + 2)` that `Monophilic.gtheta` uses. It is the exact analogue of the
`getVert` bridge at the top of `Monophilic.RubinStructure`. -/
...
of `Monophilic.contains_gtheta_of_arms` are packaged here as two predicates. Bookkeeping only. -/
...
omit [Fintype V] [DecidableEq V] [DecidableRel G.Adj] in
/-- `Monophilic.contains_gtheta_of_arms` with its hypotheses packaged as
`Monophilic.IsArm` and `Monophilic.ArmsDisj`. -/
...
omit [Fintype V] [DecidableEq V] [DecidableRel G.Adj] in
/-- **A path walk is an arm**, indexed by `Monophilic.getVert`. -/
...
This is the bridge Rubin's steps 4–6 need in order to feed `Monophilic.choosable_two_gtheta_iff`
and `Monophilic.not_choosable_two_gtheta_of_four`, which speak about the abstract graph
`Monophilic.gtheta ks`. It is the analogue for thetas of the `getVert` bridge at the top of
`Monophilic.RubinStructure`, and like it is **pure mechanization**: no mathematics happens here,
...
The arms may be given in any order: the sorting `Monophilic.ValidArms` asks for is done here.
...
`Monophilic.choosable_two_gtheta_iff`. -/
...
`Monophilic.not_choosable_two_of_three_arms` disposes of every shape but `θ_{2,2,2m}`; what is
left is to see that the two arcs are then the *short* arms, which is where minimality of `C₁`
enters (see the note on step 5 in `Monophilic.RubinStructure`). -/
...
below are Rubin's (iii)–(vi); his (i) and (ii) are `Monophilic.case_onP1` here, which does not
produce a theta but two cycles meeting in at most one vertex, and so is discharged by his step 3
(see the note on cases (i) and (ii) in `Monophilic.RubinStructure`).
...
`Monophilic.RubinStructure`), since four arms are already type `5`. -/
...
`Monophilic.not_choosable_two_of_dumbbell` covers together.
...
in `Monophilic.RubinStructure`). They are nevertheless discharged here directly, which is
...
Rubin's six cases on the ends of the second connecting path `P₂` are `Monophilic.case_onP1`
(his (i) and (ii)), `Monophilic.case_interior` (iii), `Monophilic.case_mixed` (iv),
`Monophilic.case_branches` (v) and `Monophilic.case_middles` (vi).
...
#print axioms Monophilic.gArmOf_gsize_take_add
#print axioms Monophilic.gDecode
#print axioms Monophilic.armStepB_iff
#print axioms Monophilic.gAdjAux_iff
#print axioms Monophilic.gAdjB_iff
#print axioms Monophilic.contains_gtheta_of_arms
#print axioms Monophilic.contains_gtheta_of_armFam
#print axioms Monophilic.contains_gtheta_three
#print axioms Monophilic.contains_gtheta_four
#print axioms Monophilic.isArm_of_walk
#print axioms Monophilic.armsDisj_of_walks
#print axioms Monophilic.contains_gtheta_of_walks
#print axioms Monophilic.not_choosable_two_of_three_arms
#print axioms Monophilic.not_choosable_two_of_arm_one
#print axioms Monophilic.not_choosable_two_of_four_arms
#print axioms Monophilic.getVert_mem_edges
#print axioms Monophilic.exists_arcs_of_cycle
#print axioms Monophilic.exists_isCycle_four
#print axioms Monophilic.step4
#print axioms Monophilic.isArm_edge
#print axioms Monophilic.isArm_two
#print axioms Monophilic.isArm_three
#print axioms Monophilic.IsArm.inj_all
#print axioms Monophilic.IsArm.rev
#print axioms Monophilic.case_branches
#print axioms Monophilic.case_middles
#print axioms Monophilic.case_mixed
#print axioms Monophilic.case_interior
#print axioms Monophilic.case_onP1
#print axioms Monophilic.step56
#print axioms Monophilic.rubin_structure
