...
* `Monophilic.pathGCongr` : paths of equal length are isomorphic (transport along `m = n`)
* `Monophilic.pathSplitIso` :
  `pathG (a + b + 1) ≃g bridge (pathG a) (pathG b) (pathStart a) (pathEnd b)`
* `Monophilic.pathSplitIso_pathEnd`, `Monophilic.pathSplitIso_pathStart` : where the two terminal
  vertices of the long path go
* `Monophilic.adj_pathSplitIso_symm` : the bridging edge is an edge of the path
* `Monophilic.col_pathG_split` : the induced identity of list-coloring counts,
  `col(P_{a+b+1}, M) = col(bridge, M ∘ (pathSplitIso a b).symm)`
* `Monophilic.col_bridge_pathSplit` : its converse, transporting a list assignment of the bridged
...
Everything up to `Monophilic.pathGCongr` is stated for arbitrary graphs. The point of that is that
`Monophilic.PathV` — whose `Option` structure is visible only at default transparency, `PathV` being
defined by recursion on the length — never has to be unfolded by `simp` or `rw`. -/
...
/-- The bijection underlying `Monophilic.pathSplitIso`: it cuts the `b + a + 2` vertices of a path
