...
* `Monophilic.hasCore` (`Monophilic/CoreExtract.lean`) produces the core `H` of a connected `G`,
  which is connected and either a single vertex or of minimum degree at least `2`;
* `Monophilic.choosable_iff_of_coreIs` (`Monophilic/Theorem2.lean`) says `G` is `2`-choosable
  exactly when `H` is;
* `Monophilic.rubin_structure` (`Monophilic/RubinCases.lean`) — Rubin's steps 1–6 — says that a
...
* `Monophilic.exists_iso_closePath_of_two_regular` and `Monophilic.nonempty_iso_thetaGen_of_paths`
  (`Monophilic/CoreExtract.lean`) turn that walk data into isomorphisms;
* `Monophilic.choosable_two_of_rubinAlternatives` (`Monophilic/Theorem2.lean`) is the ⟸ direction.

What is genuinely proved here, rather than merely quoted, is the glue: transporting
`Monophilic.CoreIs` along an isomorphism of the core, the counting argument that a graph of minimum
degree `≥ 2` whose edges all lie on one cycle is `2`-regular, and the bookkeeping that presents the
four-cycle-plus-path of the theta case as three internally disjoint arms.

Because `Monophilic/Theorem2.lean` is *upstream* of `Monophilic/CoreExtract.lean` and
`Monophilic/RubinCases.lean` — `Monophilic.HasCore` is stated in terms of `Monophilic.CoreIs` — the
unconditional form of Kirov–Naimi's Theorem 2 has to be stated here rather than there;
`Monophilic.monophilic_two_iff_of_rubin` is applied to `Monophilic.rubinTheorem` with no other
change.

## Main results

* `Monophilic.CoreIs.congr` : `Monophilic.CoreIs` only sees the core up to isomorphism
* `Monophilic.degree_eq_two_of_cycle_edges` : minimum degree `≥ 2` plus "every edge lies on the
  cycle `c`" forces `2`-regularity
* `Monophilic.exists_iso_theta_of_thetaData` : the theta case of Rubin's structure theorem, as an
  isomorphism onto `Monophilic.theta m`
* `Monophilic.rubinTheorem` : **Rubin's theorem**
* `Monophilic.monophilic_two_iff` : **Theorem 2 of Kirov–Naimi, unconditionally**
-/
...
/-- A graph with exactly one vertex is `Monophilic.pathG 0`. -/
...
/-- Every edge of a walk is a step of it: the converse of `Monophilic.getVert_mem_edges`. -/
...
produced by `Monophilic.rubin_structure` — a four-cycle `a, u, b, w` together with an even path
...
The assembly. The ⟸ direction is `Monophilic.choosable_two_of_rubinAlternatives`. For ⟹, pass to
the core (`Monophilic.hasCore`), which is `2`-choosable because `G` is
(`Monophilic.choosable_iff_of_coreIs`). If the core is a single vertex we are in the first case.
Otherwise it has minimum degree at least `2`, so `Monophilic.rubin_structure` applies and returns
either a spanning cycle of even length — whence the core is `2`-regular
(`Monophilic.degree_eq_two_of_cycle_edges`) and so a cycle
(`Monophilic.exists_iso_closePath_of_two_regular`), necessarily an even one since an odd cycle is
not `2`-choosable — or a labelled `θ_{2,2,2m}` (`Monophilic.exists_iso_theta_of_thetaData`). -/
...
theorem** — is now `Monophilic.rubinTheorem` rather than a hypothesis.

This is literally `Monophilic.monophilic_two_iff_of_rubin` applied to `Monophilic.rubinTheorem`;
the proof of Theorem 2 itself is in `Monophilic/Theorem2.lean` and is unchanged. It has to be
stated in this file rather than there only because `Monophilic/Theorem2.lean` is upstream of the
files Rubin's theorem is proved from. -/
theorem monophilic_two_iff {V : Type} [Fintype V] [DecidableEq V] (G : SimpleGraph V)
    [DecidableRel G.Adj] (hconn : G.Connected) :
    G.Monophilic 2 ↔ CoreIsVertex G ∨ CoreIsCycle G ∨ CoreIsK23 G ∨ HasOddCycle G :=
  monophilic_two_iff_of_rubin rubinTheorem G hconn
...
#print axioms Monophilic.exists_pendantTower_iso
#print axioms Monophilic.CoreIs.congr
#print axioms Monophilic.nonempty_iso_pathG_zero
#print axioms Monophilic.exists_getVert_of_mem_edges
#print axioms Monophilic.false_of_triangle_of_choosable_two
#print axioms Monophilic.degree_eq_two_of_cycle_edges
#print axioms Monophilic.mem_support_of_cycle_four
#print axioms Monophilic.exists_iso_theta_of_thetaData
#print axioms Monophilic.rubinTheorem
#print axioms Monophilic.monophilic_two_iff
