...
/-!
# An explicit monophilicity threshold, and Donner's theorem

Donner proved in 1992 that every graph is `k`-monophilic for all sufficiently large `k`; his proof
is non-constructive and gives no threshold.  This file proves an explicit — if crude — threshold,

`k > 2 ^ m  ⟹  G is k-monophilic`,   `m = |E(G)|`,
...
* `SimpleGraph.monophilic_of_two_pow_lt` : `2 ^ m < k → G.Monophilic k`
* `SimpleGraph.exists_monophilic_forall_ge` : Donner's theorem
...
the number of edges, then `G` is `k`-monophilic: no `k`-list assignment has fewer colorings than
the constant one.
...
theorem monophilic_of_two_pow_lt {G : SimpleGraph V} [DecidableRel G.Adj] {k : ℕ}
    (hk : 2 ^ #G.edgeFinset < k) : G.Monophilic k := by
  classical
  intro L hL
...
past some threshold the constant list assignment always minimizes the number of colorings.

Donner's proof is by an algebraic-geometry / compactness argument and gives no bound.
`monophilic_of_two_pow_lt` is an explicit form of the same statement, with the (very crude)
...
theorem exists_monophilic_forall_ge (G : SimpleGraph V) [DecidableRel G.Adj] :
    ∃ N, ∀ k, N ≤ k → G.Monophilic k :=
  ⟨2 ^ #G.edgeFinset + 1, fun _ hk => monophilic_of_two_pow_lt (by omega)⟩
...
theorem listColorFunction_eq_of_two_pow_lt {G : SimpleGraph V} [DecidableRel G.Adj] {k : ℕ}
  ...
  (monophilic_iff_listColorFunction_eq k).mp (monophilic_of_two_pow_lt hk)

/-- The same, against the genuine chromatic polynomial of `Monophilic.ChromaticPolynomial`: for
...
theorem listColorFunction_eq_eval_of_two_pow_lt {G : SimpleGraph V} [DecidableRel G.Adj] {k : ℕ}
  ...
  (monophilic_iff_listColorFunction_eq_eval k).mp (monophilic_of_two_pow_lt hk)
...
-- monophilicity itself, on the same data: `col(G, L) ≥ col(G, k)`
...
edges put the threshold of `monophilic_of_two_pow_lt` at `2 ^ 3 = 8`. -/
...
#print axioms SimpleGraph.monophilic_of_two_pow_lt
#print axioms SimpleGraph.exists_monophilic_forall_ge
