...
bipartite graph that is `n`-colorable but not `n`-choosable. This chapter formalizes that block.
...
`n`, so that the failure cannot be explained away by non-choosability. That construction, which
stacks `n²` copies of this block underneath a complete bipartite graph, is not yet formalized.
