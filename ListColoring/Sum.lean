...
The number of list colorings is multiplicative over disjoint unions. Kirov–Naimi use this implicitly
throughout (it is the reason "a graph is enumeratively chromatic-choosable at `n` iff each of its
connected components is", stated without proof on p. 1), and explicitly in the proof of Lemma 2,
where a graph is cut along a bridge into two sides.

## Main results

* `SimpleGraph.col_sum` : `col(G ⊔ H, M) = col(G, M∘inl) * col(H, M∘inr)`
* `SimpleGraph.colConst_sum` : the same for constant list assignments
* `SimpleGraph.ECCAt.sum` : a disjoint union of graphs that are enumeratively
...
/-- A disjoint union of graphs that are enumeratively chromatic-choosable at `n` is enumeratively
chromatic-choosable at `n`. -/
theorem ECCAt.sum {n : ℕ} (hG : G.ECCAt n) (hH : H.ECCAt n) :
    (G ⊕g H).ECCAt n := by
  intro M hM
