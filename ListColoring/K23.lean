...
/-!
# Lemma 6 of Kirov–Naimi: `K₂,₃` is enumeratively chromatic-choosable at `2`

Kirov–Naimi single out `K₂,₃` (Lemma 6) as a base case in their analysis of the graphs that are
enumeratively chromatic-choosable at `2`. We model it as Mathlib's
`completeBipartiteGraph (Fin 2) (Fin 3)`, a graph on `Fin 2 ⊕ Fin 3`.
...
* `SimpleGraph.ecc_K23` : **Lemma 6**, `K₂,₃` is enumeratively chromatic-choosable at `2`
-/
...
this says exactly that `K₂,₃` is enumeratively chromatic-choosable at `2`. -/
...
/-- **Lemma 6 of Kirov–Naimi.** `K₂,₃` is enumeratively chromatic-choosable at `2`: every
assignment of two-element color lists admits at least `col(K₂,₃, 2) = 2` proper colorings. -/
theorem ecc_K23 :
    (completeBipartiteGraph (Fin 2) (Fin 3)).ECCAt 2 := by
  intro L hL
