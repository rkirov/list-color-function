...
is `ListColoring.IsNNAssign k m`. Parts (b) and (c) read:
...
* **Existence of a minimizer** (`SimpleGraph.exists_minimizing`). The set of *assignments* is
  ...
  is needed.
* **Shapes** (`ListColoring.IsPathShape`). An `(n, n-1)`-assignment is *shaped* by `(S, x, y)` when
  ...
  `ListColoring.col_of_isPathShape` computes the count of any shaped assignment as `A_k` or `B_k`,
  by exhibiting it as a renaming of the canonical `ListColoring.pathAssign` (`exists_palette` plus
  `SimpleGraph.col_image_of_injOn` and `ListColoring.col_pathAssign`).
* **Nestedness** (`ListColoring.nested_of_minimizing`). If a minimizing assignment had incomparable
  lists at the two ends of some edge, cutting the path there (`ListColoring.pathSplitIso`) and
...
* Nestedness forces all interior lists to coincide and both terminal lists to sit inside them, i.e.
  a shape (`ListColoring.exists_isPathShape_of_minimizing`), whence (b) and (c).
...
`ListColoring.not_parity_of_minimizing_length_zero`. Part (b) is unaffected and is proved for all
`k` and all `n = m + 2` (for `n = 2` it is vacuous: `min(A_k, B_k) = 0`).

## Main results

* `SimpleGraph.Minimizing`, `SimpleGraph.exists_minimizing` : minimizing assignments and their
  existence
* `ListColoring.IsNNAssign`, `ListColoring.IsPathShape` : `(n, n-1)`-assignments and their type A /
  type B shapes
* `ListColoring.col_of_isPathShape` : Lemma 3(a) off the canonical assignment
* `ListColoring.nested_of_minimizing`, `ListColoring.exists_isPathShape_of_minimizing` : a
  minimizing assignment is nested at every edge, hence type A or type B
* `ListColoring.min_pathA_pathB_le_col` : **Lemma 3(b)**
* `ListColoring.col_eq_min_of_minimizing`, `ListColoring.isPathShape_parity_of_minimizing` :
  **Lemma 3(c)**
* `ListColoring.not_parity_of_minimizing_length_zero` : the counterexample to Lemma 3(c) at `k = 0`
-/
...
the path there (`ListColoring.pathSplitIso`) and apply the strict form of the swap step
...
The hypothesis `1 ≤ k` cannot be dropped; see
`ListColoring.not_parity_of_minimizing_length_zero`. -/
...
(`ListColoring.IsPathShape.eq_of_length_zero`), while `0` is even. So the conclusion of Lemma 3(c),
