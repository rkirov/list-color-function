...
`ListColoring.CycleTwo` reduces the case `n = 2` of Theorem 1 of Kirov–Naimi, for a cycle on an even
...
* `ListColoring.pathVtxEquiv` : the numbering `pathVtx k` of the vertices of the path, as an
  equivalence `Fin (k+1) ≃ PathV k` (with a computable inverse `ListColoring.pathIdx`)
* `ListColoring.pathG_adj_pathVtx` : `pathVtx k i ~ pathVtx k j` in `pathG k` iff `|i - j| = 1`
* `ListColoring.closePath_adj_pathVtx` : `pathVtx k i ~ pathVtx k j` in `closePath k` iff `i` and
  `j` are *cyclically* adjacent, `(i + 1) % (k+1) = j ∨ (j + 1) % (k+1) = i`
* `ListColoring.rotIso` : rotation by `r`, an automorphism `closePath k ≃g closePath k`
* `ListColoring.two_le_col_closePath_of_not_const` : **`2 ≤ col(C, L)`** for every non-constant
  `2`-list assignment on a cycle — no parity hypothesis needed
* `ListColoring.two_le_col_closePath_of_ends_eq` : the missing hypothesis `H` of
  `ListColoring.ecc_closePath_two_of_odd`, proved
* `ListColoring.ecc_closePath_two` : **the case `n = 2` of Theorem 1**, unconditionally
* `ListColoring.ecc_closePath_of_two_le` : **Theorem 1 in full**, every cycle is
  enumeratively chromatic-choosable at `n` for every `n ≥ 2`
-/
...
different lists at its two ends, and a rotation `ListColoring.rotIso` carries that edge onto the
distinguished edge `pathEnd k — pathStart k`, where
`ListColoring.two_le_col_closePath_of_ends_ne` applies. No parity hypothesis is needed. -/
...
/-- The hypothesis `H` of `ListColoring.ecc_closePath_two_of_odd`, now proved: a non-constant
...
enumeratively chromatic-choosable at `2`; `closePath k` has `k + 1` vertices, so `Odd k` says
that number is even. -/
theorem ecc_closePath_two {k : ℕ} (hk : Odd k) : (closePath k).ECCAt 2 :=
  ecc_closePath_two_of_odd hk fun _ hcard _ hnc =>
    two_le_col_closePath_of_not_const hk.pos hcard hnc

/-- **Theorem 1 of Kirov–Naimi in full: every cycle is enumeratively chromatic-choosable at `n`, for
every `n ≥ 2`.**

`closePath k` is the cycle on `k + 1` vertices, so `2 ≤ k` says it has at least three. For `n ≥ 3`
this is `ListColoring.ecc_closePath`; the case `n = 2` splits into an odd number of vertices
(`ListColoring.ecc_closePath_of_odd`, where `col(C, 2) = 0`) and an even number of vertices
(`ListColoring.ecc_closePath_two`). -/
  ...
  · rcases Nat.even_or_odd k with hev | hodd
    · obtain ⟨j, rfl⟩ : ∃ j, k = j + 1 := ⟨k - 1, by omega⟩
      have hj : Odd j := by
        ...
        · exact h
      exact ecc_closePath_of_odd hj 0
    · exact ecc_closePath_two hodd
  · exact ecc_closePath hk hm

end ListColoring

#print axioms ListColoring.pathVtxEquiv
#print axioms ListColoring.pathG_adj_pathVtx
#print axioms ListColoring.closePath_adj_pathVtx
#print axioms ListColoring.closePath_adj_pathVtx_fin
#print axioms ListColoring.rotIso
#print axioms ListColoring.two_le_col_closePath_of_not_const
#print axioms ListColoring.two_le_col_closePath_of_ends_eq
#print axioms ListColoring.ecc_closePath_two
#print axioms ListColoring.ecc_closePath_of_two_le
