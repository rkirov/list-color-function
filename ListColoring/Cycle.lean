...
assignment. Since `pathA m k` counts the colorings of such a path (`ListColoring.col_pathAssign`),
summing over the `n` choices of `c` gives the paper's `col(C, n) = n · A_{k-2}` for a cycle on `k`
vertices.

Following `ListColoring.Path`, a cycle is built as the path of length `k` (so `k + 1` vertices) with
...
`ListColoring.Delete` applies with no transport. For `k ≥ 2` this really is the cycle on `k + 1`
...
* `ListColoring.closePath k` : `pathG k` together with the edge `pathStart k — pathEnd k`

## Main results

* `ListColoring.closePath_adj` : `closePath k` is `pathG k` plus that one edge and nothing else
* `ListColoring.delNone_closePath_succ` : deleting `pathEnd` from `closePath (k+1)` leaves `pathG k`
* `ListColoring.inducedList_closePath_constList` : the list assignment induced on that path by
  giving the color `c` to the deleted vertex is the type A assignment `pathAssign k n c c`
* `ListColoring.colFix_closePath_succ` : consequently
  `col(C, n, pathEnd, c) = col(P_k, pathAssign k n c c)`
* `ListColoring.colConst_closePath` : `col(C, n) = n · A_{k-1}` for the cycle `closePath k`
-/
