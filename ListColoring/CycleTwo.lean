...
`ListColoring.CycleECC` proves Theorem 1 for every `n ≥ 3`, and for `n = 2` on cycles with an
**odd** number of vertices (`ListColoring.ecc_closePath_of_odd`, where the count `col(C, 2)`
...
(`ListColoring.colorings_closePath_eq_filter`).

For a `2`-list assignment `L` it therefore suffices to show `2 ≤ col(closePath k, L)`, since
`col(closePath k, 2) = 2` for odd `k` (`ListColoring.colConst_closePath_two_of_odd`).

* If `L` is **constant** this is an equality, by renaming invariance
  (`ListColoring.col_closePath_const`).
* If `L` is **not** constant, the paper's *forcing* argument applies. Write
  ...
  (`ListColoring.const_of_forall_reach_card_one`) shows that if **every** color of `L (pathEnd k)`
...
* `ListColoring.colConst_closePath_two_of_odd` : `col(C, 2) = 2` for a cycle on an even number of
  vertices
* `ListColoring.colorings_closePath_eq_filter` : cycle colorings are the path colorings with
  distinct colors at the two ends
* `ListColoring.const_of_forall_reach_card_one` : **the propagation lemma** — if every color of
  `L (pathEnd k)` forces a color of `L (pathStart k)`, then `L` is constant
* `ListColoring.exists_reach_eq_pathStart` : consequently some color of `L (pathEnd k)` forces
  nothing
* `ListColoring.two_le_col_closePath_of_ends_ne` : **`2 ≤ col(C, L)`** whenever the two ends of the
  removed edge carry different lists
* `ListColoring.col_closePath_const` : the constant case

## What is missing

The one configuration not covered here is a non-constant `L` with
`L (pathEnd k) = L (pathStart k)`. See the note before
`ListColoring.two_le_col_closePath_of_ends_ne` for exactly where the paper's argument uses
...
#print axioms ListColoring.colConst_closePath_two_of_odd
#print axioms ListColoring.mem_colorings_closePath_iff
#print axioms ListColoring.colorings_closePath_eq_filter
#print axioms ListColoring.col_closePath_const
#print axioms ListColoring.ne_pathStart_of_ne_pathEnd
#print axioms ListColoring.const_of_forall_reach_card_one
#print axioms ListColoring.exists_reach_eq_pathStart
#print axioms ListColoring.two_le_col_closePath_of_ends_ne
#print axioms ListColoring.ecc_closePath_two_of_odd
