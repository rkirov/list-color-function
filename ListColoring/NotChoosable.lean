...
`(n+1)`-choosable but not enumeratively chromatic-choosable at `n+1` — are assembled out of copies
of a complete bipartite graph carrying a list assignment with **no** colorings at all. It is also
the classical Erdős–Rubin–Taylor separation between colorability and choosability: bipartite graphs
have chromatic number `2`, but their choice number is unbounded.
...
* `SimpleGraph.ERT.not_ecc` : hence `K_{n,nⁿ}` is not enumeratively chromatic-choosable at `n` for
  `2 ≤ n`
...
/-- Since `K_{n,nⁿ}` is `n`-colorable but not `n`-choosable, it is not enumeratively
chromatic-choosable at `n`: it sits in the middle regime `χ(G) ≤ n < χ_ℓ(G)` of Kirov–Naimi p. 2. -/
theorem not_ecc (hn : 2 ≤ n) : ¬ (K n).ECCAt n :=
  not_ecc_of_colorable_of_not_choosable (colorable n hn) (not_choosable n)
