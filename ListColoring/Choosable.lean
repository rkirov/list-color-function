...
`ListColoring.Basic` defines `SimpleGraph.Choosable G n` — every `n`-list assignment admits a proper
coloring — and relates it to `SimpleGraph.Colorable` and to enumerative chromatic-choosability at
`n`. This file develops the structural theory of that predicate: how it transfers along
isomorphisms, how it behaves under adding or removing edges and vertices, and what it says about the
two smallest graphs that matter for `n = 2`.
...
(`SimpleGraph.ecc_two_iff_of_rubin` in `ListColoring.Theta`), and the results below are the
...
* the "core" half of Rubin's statement — repeatedly deleting vertices of degree one changes
  ...
  Lemma 5 is proved in `ListColoring.Core`: a pendant vertex is a cone over a singleton, and a cone
...
* `ListColoring.choosable_pathG` : every path is `n`-choosable for `n ≥ 2`, by the pendant step
* `ListColoring.choosable_two_closePath_of_odd` : a cycle on an even number of vertices is
  `2`-choosable
* `ListColoring.not_choosable_two_closePath_of_even` : a cycle on an odd number of vertices is not
-/
...
count the colorings. Compare `SimpleGraph.ecc_congr`. -/
...
`ListColoring.Core` for enumerative chromatic-choosability.
...
Iterating the pendant step over the tower construction of `ListColoring.Core` gives the choosability
...
`SimpleGraph.ecc_pendantTower_iff`, and is the step of Rubin's theorem that reduces a
...
This is a corollary of Theorem 1 of Kirov–Naimi (`ListColoring.ecc_closePath_of_two_le`):
enumerative chromatic-choosability at `2` says that the constant list assignment *minimizes* the
number of colorings, and that minimum is `col(C, 2) = 2` by
`ListColoring.colConst_closePath_two_of_odd`. So every `2`-list assignment admits at least two
colorings, in particular at least one. -/
theorem choosable_two_closePath_of_odd {k : ℕ} (hk : Odd k) (hk2 : 2 ≤ k) :
    (closePath k).Choosable 2 := by
  intro L hL
  have h := ecc_closePath_of_two_le (m := 0) hk2 L hL
...
(`ListColoring.pathA_pathB_zero_parity`), and `col(C, 2) = 2 · A_{k-1}` vanishes precisely when
`k - 1` is odd. -/
...
Together with `ListColoring.choosable_two_closePath_of_odd` this pins down exactly which cycles
...
#print axioms ListColoring.choosable_pathG
#print axioms ListColoring.choosable_two_closePath_of_odd
#print axioms ListColoring.colConst_closePath_two_of_even
#print axioms ListColoring.not_colorable_two_closePath_of_even
#print axioms ListColoring.not_choosable_two_closePath_of_even
#print axioms ListColoring.choosable_closePath_zero
