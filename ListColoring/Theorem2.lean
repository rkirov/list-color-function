...
The paper's "`2`-monophilic" is the property the literature now calls *enumeratively
chromatic-choosable at `2`*; that is the name this development uses (`SimpleGraph.ECCAt`, see
`ListColoring.Defs`), and Theorem 2 below is stated with it.
...
propositions, and proves it from one hypothesis: **Rubin's theorem** (`ListColoring.RubinTheorem`).
The hypothesis is the *first explicit argument* of `ListColoring.ecc_two_iff_of_rubin`, so that a
term `rubinTheorem : RubinTheorem` turns it into the unconditional theorem with no other edit
...
quotient. Kirov–Naimi's Lemma 5 (`SimpleGraph.ecc_pendantTower_iff`) is what makes the two
readings interchangeable, and it is the paper's, not ours.

### A parity convention that bites

`ListColoring.closePath k` is the cycle on `k + 1` vertices. So `Odd k` picks out the cycles on an
...
* the whole **⟸ direction** of Theorem 2 (`ListColoring.ecc_two_of_alternatives`);
* in the **⟹ direction**, the case where `G` is *not* `2`-colorable
  (`ListColoring.hasOddCycle_of_not_colorable_two`: a graph that is not `2`-colorable contains an
  odd cycle — the converse of the easy bridge, and the one ingredient of the paper's proof that
  Mathlib did not have);
* in the **⟹ direction**, the reduction of the `2`-colorable case to Rubin's *conclusion*
  (`ListColoring.alternatives_of_rubinAlternatives`), which consumes the three-way disjunction and
  produces the four-way one without assuming Rubin;
* the **⟸ direction of Rubin's theorem** itself
  (`ListColoring.choosable_two_of_rubinAlternatives`), which is what
  `ListColoring.rubinTheorem` consumes for that half, leaving it to supply only the forward
  implication.
...
* `ListColoring.CoreIsEvenCycle`, `ListColoring.CoreIsTheta` : the other two alternatives of Rubin's
  theorem
* `ListColoring.RubinTheorem` : Rubin's theorem, as a proposition

## Main results

* `SimpleGraph.ecc_of_forall_not_adj` : an edgeless graph is enumeratively chromatic-choosable at
  `n`
* `ListColoring.not_colorable_of_contains` : non-`n`-colorability passes from a subgraph upwards
* `ListColoring.exists_odd_isCycle_of_odd_closed_walk` : an odd closed walk contains an odd cycle
* `ListColoring.hasOddCycle_of_not_colorable_two` : **a graph that is not `2`-colorable contains an
  odd cycle**
* `ListColoring.ecc_two_of_alternatives` : the ⟸ direction of Theorem 2
* `ListColoring.alternatives_of_rubinAlternatives` : the ⟹ direction, `2`-colorable case
* `ListColoring.ecc_two_iff_of_rubin` : **Theorem 2 of Kirov–Naimi**
-/
...
/-- **An edgeless graph is enumeratively chromatic-choosable at `n`**, for every `n`: every list
assignment, constant or not, admits exactly `∏ v, |L v|` colorings, and for an `n`-list assignment
that is `n ^ |V|` either way.
...
theorem ecc_of_forall_not_adj {n : ℕ} (h : ∀ a b, ¬ G.Adj a b) : G.ECCAt n := by
...
an odd *cycle* from an odd closed walk — is `ListColoring.exists_odd_isCycle_of_odd_closed_walk`
below, and the translation between `SimpleGraph.cycleGraph` and this development's
`ListColoring.closePath` is `ListColoring.contains_closePath_of_isContained_cycleGraph`. -/
...
`ListColoring.not_choosable_of_contains`. -/
...
/-- The numbering `ListColoring.pathVtx` identifies `closePath (k+1)` with Mathlib's
...
`ListColoring.closePath (k+1)`: the two are the same cycle, numbered differently.
...
(`ListColoring.not_colorable_two_closePath_of_even`, itself a corollary of Kirov–Naimi's Theorem
1 in this development) and non-colorability passes upwards. -/
...
theorem ecc_iff_of_coreIs (h : CoreIs G H) (n : ℕ) : G.ECCAt n ↔ H.ECCAt n := by
  ...
  exact (ecc_iso e n).trans (ecc_pendantTower_iff n k d)
...
imports this one — so `ListColoring.ecc_two_iff_of_rubin` takes a term of this type as its
first explicit argument, and `ListColoring.ecc_two_iff` supplies it. The `↔` form is the form
the argument is stated in; its ⟸ half is proved here, as
`ListColoring.choosable_two_of_rubinAlternatives`. -/
...
/-- **`θ_{2,2,2} = K₂,₃` is enumeratively chromatic-choosable at `2`.** This is Kirov–Naimi's
**Lemma 6** (`SimpleGraph.ecc_K23`), moved onto the theta model. -/
theorem ecc_two_theta_one : (theta 1).ECCAt 2 :=
  (ecc_iso k23IsoThetaOne 2).mp ecc_K23

/-- **The one-vertex graph is enumeratively chromatic-choosable at `n`.** It has no edges. Not
from the literature — the degenerate case, which Kirov–Naimi dispose of with the word "clearly". -/
theorem ecc_pathG_zero (n : ℕ) : (pathG 0).ECCAt n :=
  ecc_of_forall_not_adj fun _ _ h => h

/-- A graph whose core is a single vertex is enumeratively chromatic-choosable at `2` — indeed
enumeratively chromatic-choosable at `n` for every `n`. By Kirov–Naimi's **Lemma 5** it suffices
that the core is, and a one-vertex graph has no edges. -/
theorem ecc_of_coreIsVertex (h : CoreIsVertex G) (n : ℕ) : G.ECCAt n :=
  (ecc_iff_of_coreIs h n).mpr (ecc_pathG_zero n)

/-- A graph whose core is a cycle is enumeratively chromatic-choosable at `2` — indeed
enumeratively chromatic-choosable at `n` for every `n`. This is Kirov–Naimi's **Theorem 1**
(`ListColoring.ecc_closePath_all`: every cycle is enumeratively chromatic-choosable at `n`)
together with **Lemma 5**. Note that no parity hypothesis is needed: *every* cycle is
enumeratively chromatic-choosable at `n`, which is why Theorem 2 says "is a cycle" where Rubin's
theorem says "is an even cycle". -/
theorem ecc_of_coreIsCycle (h : CoreIsCycle G) (n : ℕ) : G.ECCAt n := by
  obtain ⟨k, hk2, hc⟩ := h
  exact (ecc_iff_of_coreIs hc n).mpr (ecc_closePath_all k n hk2)

/-- A graph whose core is `K₂,₃` is enumeratively chromatic-choosable at `2`. Kirov–Naimi's
**Lemma 6** together with **Lemma 5**. -/
theorem ecc_two_of_coreIsK23 (h : CoreIsK23 G) : G.ECCAt 2 :=
  (ecc_iff_of_coreIs h 2).mpr ecc_two_theta_one

/-- A graph containing an odd cycle is enumeratively chromatic-choosable at `2`, vacuously: it is
not `2`-colorable, so it has no colorings at all from the constant list of two colors. This is
item (2) of the three regimes on p. 2 of Kirov–Naimi (`SimpleGraph.ecc_of_not_colorable`). -/
theorem ecc_two_of_hasOddCycle (h : HasOddCycle G) : G.ECCAt 2 :=
  ecc_of_not_colorable (not_colorable_two_of_hasOddCycle h)

/-- **The ⟸ direction of Kirov–Naimi's Theorem 2, unconditionally.** A graph whose core is a single
vertex, a cycle or `K₂,₃`, or which contains an odd cycle, is enumeratively chromatic-choosable at
`2`.

This is the direction the paper disposes of in two sentences: "Clearly a single vertex and a
graph that contains an odd cycle are both enumeratively chromatic-choosable at `2`. Also, by
Theorem 1 and Lemma 6, all cycles and `K₂,₃` are enumeratively chromatic-choosable at `2`. Using
Lemma 5, this gives us one direction of the theorem." Nothing is assumed here — in particular
Rubin's theorem is not used, and neither is connectivity. -/
theorem ecc_two_of_alternatives
    (h : CoreIsVertex G ∨ CoreIsCycle G ∨ CoreIsK23 G ∨ HasOddCycle G) : G.ECCAt 2 := by
  rcases h with h | h | h | h
  · exact ecc_of_coreIsVertex h 2
  · exact ecc_of_coreIsCycle h 2
  · exact ecc_two_of_coreIsK23 h
...
`ListColoring.choosable_two_of_rubinFamily`, itself a corollary of Kirov–Naimi's Theorem 1 in the
even-cycle case. It is the ⟸ half of `ListColoring.rubinTheorem`, which is discharged by citing this
...
Given that `G` is enumeratively chromatic-choosable at `2` and that Rubin's *conclusion* holds for
it — its core is a single vertex, an even cycle or a `θ_{2,2,2m}` — the four alternatives of Theorem
2 follow. This is the last sentence of the paper's proof: "So, by Rubin's theorem above, it is
enough to show that for `m ≥ 2`, `θ_{2,2,2m}` is not enumeratively chromatic-choosable at `2`."

Rubin's theorem is *not* assumed here; its conclusion is taken as a hypothesis, so that the only
place `ListColoring.RubinTheorem` is consumed in this file is
`ListColoring.ecc_two_iff_of_rubin`. The two moving parts are the collapse of "even cycle" into
"cycle" — Theorem 2 asks for less — and the elimination of `θ_{2,2,2m}` for `m ≥ 2` by
`ListColoring.not_ecc_theta`, leaving `m = 1`, which is `K₂,₃`. -/
theorem alternatives_of_rubinAlternatives (hmono : G.ECCAt 2)
  ...
  · rcases Nat.lt_or_ge m 2 with hm2 | hm2
    ...
    · exact absurd ((ecc_iff_of_coreIs hc 2).mp hmono) (not_ecc_theta m hm2)
...
The proof follows the paper. The ⟸ direction is `ListColoring.ecc_two_of_alternatives`. For ⟹: if
`G` is not `2`-colorable it contains an odd cycle (`ListColoring.hasOddCycle_of_not_colorable_two`);
otherwise enumerative chromatic-choosability at `2` plus `2`-colorability gives `2`-choosability
(`SimpleGraph.choosable_of_ecc_of_colorable`), Rubin's theorem applies to the connected graph `G`,
and `ListColoring.alternatives_of_rubinAlternatives` converts its conclusion into the four
alternatives.
...
#print axioms SimpleGraph.ecc_of_forall_not_adj
#print axioms ListColoring.not_colorable_of_contains
#print axioms ListColoring.exists_odd_isCycle_of_odd_closed_walk
#print axioms ListColoring.contains_closePath_of_isContained_cycleGraph
#print axioms ListColoring.not_colorable_two_of_hasOddCycle
#print axioms ListColoring.hasOddCycle_of_not_colorable_two
#print axioms ListColoring.ecc_iff_of_coreIs
#print axioms ListColoring.choosable_iff_of_coreIs
#print axioms ListColoring.ecc_two_theta_one
#print axioms ListColoring.ecc_pathG_zero
#print axioms ListColoring.ecc_of_coreIsVertex
#print axioms ListColoring.ecc_of_coreIsCycle
#print axioms ListColoring.ecc_two_of_coreIsK23
#print axioms ListColoring.ecc_two_of_hasOddCycle
#print axioms ListColoring.ecc_two_of_alternatives
#print axioms ListColoring.choosable_two_of_rubinAlternatives
#print axioms ListColoring.alternatives_of_rubinAlternatives
#print axioms ListColoring.ecc_two_iff_of_rubin
