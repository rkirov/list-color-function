...
`ListColoring.col_theta` expresses `col(θ_{2,2,2m}, M)` as a sum over the colorings `g` of the long
...
(`ListColoring.card_sdiff_endPair_pos`), which in particular forces `x ≠ y`. Writing
...
(`ListColoring.exists_ends_eq_or_three_endPairs`): either some coloring identifies the two ends, or
there are three colorings with pairwise distinct end pairs. Two lists `La`, `Lb` can block at most
two of the three, whence `ListColoring.exists_path_coloring_endPair_ne`.
...
* **Constant.** With every list equal to `{x, y}` on a path of even length, the alternating
  coloring returns to `x` at the far end. This is already available: `ListColoring.col_chain_const`
  counts the colorings with `x` prescribed at *both* ends and finds exactly one.
* **Non-constant.** Then `ListColoring.exists_reach_eq_pathStart` supplies a color `α` at `pathEnd`
...
* `ListColoring.card_sdiff_endPair_pos` : `0 < #(L \ endPair k g) ↔ L ≠ endPair k g`, for `#L = 2`
* `ListColoring.card_sdiff_endPair_pos_of_ends_eq` : the same, unconditionally, when `g` identifies
  the two ends
* `ListColoring.col_theta_pos_of_endPair_ne` : **the reduction** — one good path coloring suffices
* `ListColoring.col_theta_pos_of_ends_eq` : a path coloring identifying the two ends suffices
* `ListColoring.exists_ends_eq_of_const` : the alternating coloring, for a constant assignment
* `ListColoring.exists_ends_eq_or_three_endPairs` : **the path statement**
* `ListColoring.exists_path_coloring_endPair_ne` : the path statement, as used below
* `ListColoring.choosable_theta` : **`θ_{2,2,2m}` is `2`-choosable for `m ≥ 1`**
* `ListColoring.choosable_and_not_ecc_theta` : for `m ≥ 2`, `2`-choosable but not
  enumeratively chromatic-choosable at `2`
-/
...
-- The witness of `ListColoring.thetaWitness` is another `2`-list assignment, and it too is colorable.
...
`ListColoring.col_theta` writes `col(θ_{2,2,2m}, M)` as a sum of products over the colorings of the
...
/-- For `m ≥ 2` the graph `θ_{2,2,2m}` is `2`-choosable and yet *not* enumeratively
chromatic-choosable at `2`: the two notions really are independent above the chromatic number.
This is the configuration excluded by `SimpleGraph.not_ecc_of_colorable_of_not_choosable`, seen
from the other side. -/
theorem choosable_and_not_ecc_theta (m : ℕ) (hm : 2 ≤ m) :
    (theta m).Choosable 2 ∧ ¬ (theta m).ECCAt 2 :=
  ⟨choosable_theta m (by omega), not_ecc_theta m hm⟩

end ListColoring

#print axioms ListColoring.card_sdiff_endPair_pos
#print axioms ListColoring.col_theta_pos_of_endPair_ne
#print axioms ListColoring.col_theta_pos_of_ends_eq
#print axioms ListColoring.exists_ends_eq_of_const
#print axioms ListColoring.exists_ends_eq_or_three_endPairs
#print axioms ListColoring.exists_path_coloring_endPair_ne
#print axioms ListColoring.choosable_theta
#print axioms ListColoring.colorable_theta
#print axioms ListColoring.choosable_and_not_ecc_theta
